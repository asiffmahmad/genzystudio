...
export function MetaPageSelect() {
  ...
  useEffect(() => {
    ...
    const fetchSession = async () => {
      try {
        const response = await axios.get(`http://localhost:5001/api/oauth/facebook/session/${sessionId}`);
        if (response.data.success) {
          setPages(response.data.pages);
        } else {
          setError(response.data.error || 'Session expired/invalid');
        }
      } catch (err: any) {
        if (err.response?.status === 404 || err.response?.status === 400) {
          setError('Session expired or invalid. Please reconnect your Meta account.');
        } else {
  ...
  const handleSelectPage = async (pageId: string) => {
    ...
    try {
      const response = await axios.post('http://localhost:5001/api/oauth/facebook/select-page', {
        sessionId,
        pageId
      });
      if (response.data.success) {
        ...
        alert(response.data.error || 'Failed to connect page');
