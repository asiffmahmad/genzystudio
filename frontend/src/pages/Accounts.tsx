...
export function Accounts() {
  ...
  const handleConnectClick = (platform: string) => {
    ...
    window.location.href = `http://localhost:5001/api/oauth/${endpoint}/auth`;
  };
  ...
  const submitConnection = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:5001/api/accounts/connect', {
  ...
  const handleDisconnect = async (accountId: string, platform: string) => {
    ...
    try {
      const response = await axios.post(`http://localhost:5001/api/accounts/${accountId}/disconnect`);
      if (response.data.success) {
        fetchAccounts();
      }
    } catch (error) {
                    ...
                    <div className="bg-gray-800/50 rounded-lg p-3 border border-gray-800">
                      ...
                      <button
                        onClick={async () => {
                          try {
                            setIsSubmitting(true);
                            const res = await axios.post(`http://localhost:5001/api/accounts/${connectedAccount.platform}/test`);
                            if (res.data.success) {
                              alert(`Successfully posted test message to ${connectedAccount.platform}! \nURL: ${res.data.url || 'Check your feed'}`);
                            } else {
                              alert(`Failed: ${res.data.error || JSON.stringify(res.data.details)}`);
                            }
                          } catch (err: any) {
                            alert(`Error testing post: ${err.response?.data?.error || err.message}\nDetails: ${JSON.stringify(err.response?.data?.details || {})}`);
                          } finally {
                            setIsSubmitting(false);
                          }
                        }}
                  ...
                  <>
                    ...
                    <button
                      onClick={async () => {
                        try {
                          setIsSubmitting(true);
                          const res = await axios.post(`http://localhost:5001/api/accounts/test-env`, {
                            platform: platform.name
                          });
                          if (res.data.success) {
                            alert(`Success! \nMessage: ${res.data.message}\nURL: ${res.data.url || ''}`);
                          } else {
                            alert(`API Response:\n${res.data.error || JSON.stringify(res.data.details)}`);
                          }
                        } catch (err: any) {
                          alert(`Error testing env connection: ${err.response?.data?.error || err.message}\nDetails: ${JSON.stringify(err.response?.data?.details || {})}`);
                        } finally {
                          setIsSubmitting(false);
                        }
                      }}
