...
export function ContentComposer() {
  ...
  const handleSaveDraft = async () => {
    setIsSaving(true);
    try {
      const response = await axios.post('http://localhost:5001/api/content', {
        title,
        content,
        platforms: selectedPlatforms,
        mediaUrl
      });
      setCurrentContentId(response.data.data.id);
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      const saveResponse = await axios.post('http://localhost:5001/api/content', {
        title: title || 'Quick Post',
        content,
        platforms: selectedPlatforms,
        mediaUrl
      });
      const contentId = saveResponse.data.data.id;
      setCurrentContentId(contentId);

      // 2. Publish it immediately
      const response = await axios.post(`http://localhost:5001/api/content/${contentId}/publish`);
      if (response.data.success) {
        const resultsText = response.data.results.map((r: any) => `${r.platform}: ${r.success ? 'Success ✅' : 'Failed ❌ - ' + r.error}`).join('\n');
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const response = await axios.post('http://localhost:5001/api/media/upload', formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      });
      if (response.data.success) {
        setMediaUrl(`http://localhost:5001${response.data.data.url}`);
      }
    } catch (error) {
