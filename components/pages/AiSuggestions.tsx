...
export function AiSuggestions() {
  ...
  const fetchSuggestions = async () => {
    try {
      const { getAiSuggestions } = await import('@/actions/ai');
      const response = await getAiSuggestions();
      if (response.success && response.data) {
        setSuggestions(response.data as AiSuggestion[]);
      }
    } catch (error) {
  ...
  const handleGenerate = async () => {
    ...
    setGenerating(true);
    try {
      const { generateAiSuggestion } = await import('@/actions/ai');
      const response = await generateAiSuggestion(topicInput);
      if (response.success) {
        setTopicInput('');
        fetchSuggestions();
      } else {
        alert(response.error || 'Failed to generate suggestion');
      }
    } catch (error: any) {
      console.error('Error generating suggestion', error);
      alert('Failed to generate content. Please check your GROQ_API_KEY.');
    } finally {
      setGenerating(false);
    }
  };
  ...
  const handleUseSuggestion = async (id: string) => {
    try {
      const { useAiSuggestion } = await import('@/actions/ai');
      const response = await useAiSuggestion(id);
      if (response.success) {
        router.push('/content');
      } else {
        alert(response.error || 'Failed to use suggestion');
      }
    } catch (error) {
